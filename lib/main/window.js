...
class Window {
  ...
  get instance () {
    if ( undefined === this._instance )
    {
      ...
      .on('close', (evt) => {
        ...
        console.log(`[on 'close'] La fenêtre #${this.id} (${this.name}) vient d'être closed`)
        Window.remove(this)
      })
      .on('hide', (evt) => {
        console.log(`[on 'hide'] La fenêtre #${this.id} (${this.name}) vient d'être hidée`)
        Window.on_hide(this)
      })
      .on('show', (evt) => {
        console.log(`[on show] La fenêtre #${this.id} (${this.name}) vient d'être showed`)
        Window.on_show(this)
      })
      .on('focus', (evt) => {
        console.log(`[on focus] La fenêtre #${this.id} (${this.name}) vient d'être focusée`)
  ...
  **/
  ...
  show ( callb ) {
    console.log(`-> show de ${this.relpath}`)
    ...
    {
      if(this.opened){
        console.log(`[show de ${this.relpath}] Je mets opened à false après une erreur et je détruis l'instance`)
        this.opened = false
