...
class PTestsDOM
{
  ...
  **/
  ...
  {
    let testInstance = new PTestsDOM(act, attrs_exp, options)
    return testInstance.searchExpectedInActual()
  }
  ...
  **/
  checkChildrenIn ( tag )
  {
    ...
    // Chaque enfant doit se trouver dans le tag
    // Note : `child` est une liste qui contient en premier élement la tag
    // recherchée et en second élément un objet définissant les attributs
    for( child of this.expectation.children )
    {
      hchild      = child[1]
      hchild.tag  = child[0]
      res = PTestsDOM.actualTagContainsExpect(tag, hchild)
  ...
  checkFinalCount ()
  {
    pending("à implémenter dans dom.js")
  }
}
