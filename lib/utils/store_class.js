/** ---------------------------------------------------------------------
  ...
  *   Classe Store
  *   ------------
  *   Utilisée pour lire et écrire des données dans des fichiers
  *
...
class Store
{
  ...
  **/
  saveSync ()
  {
    const my = this
    my.saving = true
    const data2save = JSON.stringify( my.owner ? my.owner.data : my.data )
    my.owner && (my.owner.saving = true)
    my.ensureFolder()
    fs.writeFileSync(my.fpath, data2save, {encoding:'utf8'})
    my.saved  = true
    my.saving = false
    if (my.owner) {
  ...
  save ()
  {
    ...
    return new Promise( (ok, ko) => {
      fs.writeFile(my.path, my.data_json, 'utf8', (err) => {
        if (err) ko(err)
        else {
          my.saving = false
          if ( my.owner )
  ...
  **/
  ...
  /**
  * Appelé seulement pour l'enregistrement des données
  **/
  get data_json ()
  {
    let d = this.owner ? this.owner.data : this.data
  ...
  **/
  ...
  load ()
  {
    ...
    let allchunks = []
    return new Promise( (ok, ko) => {
      let rstream = fs.createReadStream(this.fpath, {encoding:'utf8', highWaterMark: 32 * 1024 /*64 * 1024*/})
      rstream
        .on('data', ( chunk ) => {
  ...
  **/
  loadAndTreatSync ( methodTreatment )
  {
    const my = this
    let data = null
    if (fs.existsSync(my.fpath))
    {
      data = fs.readFileSync(my.fpath, 'utf8')
  ...
  **/
    ...
    if ( fs.existsSync(this.fpath) )
    {
      ...
      if ( this.saving ) {
        ...
        else
        {
          ...
          let code = fs.readFileSync(this.fpath)
          if (code && code != '')
          {
            this._data = JSON.parse(code)
          }
          else
          {
  ...
  // alias pour faire <owner>.store.path
  get path () { return this.fpath }
  ...
  **/
  static get app ()
  {
    ...
    {
      console.log("BASE DE L'APPLICATION", path.resolve('.'))
