...
class DOM
{
  ...
  **/
  ...
  static create (typeElement, dataElement)
  {
    ...
    let e = document.createElement(typeElement)
      , container
      , child
      , inner
    ...
    if ( undefined !== dataElement.inner ) {
      inner = dataElement.inner
      inner != '' || ( inner = null )
      dataElement.inner = undefined
    ...
    // Tout ce qui reste
    forEach(dataElement, (value, att) => {
      if ( null === value ) { return }
      e.setAttribute(att, value)
    })

    inner     && ( e.innerHTML = inner )
    child     && this.add(e,child)
    container && this.add(container, e)
