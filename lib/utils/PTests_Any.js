...
class Any {
  ...
  **/
  equals (expected, strict )
  {
    ...
    if ( this.class != expected.class ) { return false }
    ...
    switch(this.class)
    {
      ...
      default:
        ...
        res = vrai ? null : `les deux éléments ${this.class} ne sont pas strictement identiques`
    }
  ...
  **/
  static areEqual(foo1, foo2, options)
  {
    ...
    foo1 = new Any(foo1)
    foo2 = new Any(foo2)
    if ( options.strict ) { return foo1 === foo2 }
    else                  { return foo1 == foo2  }
  }
  ...
  **/
  static areEqualAsArray (any1, any2, strict)
  {
    ...
    for(; i < len ; ++i) {
      elarr1 = new Any(arr1[i])
      elarr2 = new Any(arr2[i])
      if ( ! Any.areEqual(elarr1, elarr2, strict) ) { return `le ${parseInt(i)+1}ème élément ne correspond pas : ${elarr1.inspect()} différent de ${elarr2.inspect()}` }
    }
    return true
  }
  ...
  static areEqualAsString (any1,any2,strict)
  {
    ...
    else
    {
      ...
      if ( ! str.search(expReg) ) { return 'les deux chaines ne sont pas identiques, même approximativement'}
    }
