...
class Any {
  ...
  **/
  equals (expected, strict )
  {
    ...
    // log(`*** Test classOK entre ${this.inspect()}::${this.class} ? ${expected.inspect()}::${expected.class} avec strict = ${strict}`)
    let classOK =
    ...
    // log(`     classOK est ${classOK}`)
    if ( false == classOK )
    {
      // log(`La classe n'est pas bonne entre ${this.inspect()} et ${expected.inspect()}`)
  ...
  **/
  containsAsString (element, strict, deep )
  {
    if ( strict )
    {
      return this.original.search(element.original) > -1
    }
    else
    {
      let re
      if ( 'regexp' == element.class ) { re = element.original }
      else re = new RegExp(element.original,'i')
  ...
  **/
  ...
  static areEqualAsPurObject (any1, any2, strict)
  {
    ...
    // pas être égaux.
    // log(`Nombre de keys de obj1 : ${Object.keys(obj1)}`)
    // log(`Nombre de keys de obj2 : ${Object.keys(obj2)}`)
    if ( Object.keys(obj1).length != Object.keys(obj2).length ) {
      // log("=> ON retourne l'erreur")
