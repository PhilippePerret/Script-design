...
class Any {
  ...
  **/
  diff_between (act, exp, diff_list)
  {
    ...
    if ( act.constructor.name != exp.constructor.name )
    {
      diff_list.push(`actual::${act.constructor.name}/expected::${exp.constructor.name}: imcomparables.`)
    }
    switch ( this.class ){
      case 'string':
      ...
      case 'object':
        for( p in act )
        {
          let actp_const = act[p].constructor.name // pour le message
          switch ( actp_const )
          {
            case 'String':
            case 'Number':
