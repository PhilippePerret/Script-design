  ...
  static defineTestFilesListAndRun ()
  {
    if ( undefined === this._test_file_list )
    {
      ...
      if ( this.option('test_file') ) {
        let fullpath = path.resolve(path.join(this.option('test_file')))
        ...
        if ( this.option('test_folder') )
        {
          ...
          ptests_folder = path.join('.','tests','ptests', sous_folder)
        }
        else {
          ptests_folder = path.join('.','tests','ptests')
        }

        // On traite chaque fichier
        ptests_folder = path.resolve(ptests_folder)
