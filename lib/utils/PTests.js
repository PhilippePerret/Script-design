...
class PTests {
  ...
  **/
  ...
  static require_module ( mpath )
  {
    ...
    let res = require(mpath)
    log(`retour de require(${mpath})`)
    log(res)
    return res
  }
  ...
  **/
  ...
  get exist ()
  {
    ...
    else
    {
      ...
      this.options.template.failure = `Impossible d’utiliser le test 'exist' avec ${this.actual}`
    }
    return this.writeCase()
  }
  ...
  getRealPreviousExpect ()
  {
    ...
    if ( this.expect_index ) {
      for (i = this.expect_index - 1 ; i >= 0 ; --i ){
        if(this.it.expectations[i].isTest){return this.it.expectations[i]}
      }
    }
    return undefined
  }
...
`
  ...
  defineActualString ()
  {
    ...
    if ( this.actual_str_provided )
    {
      ...
      if ( !this.options.no_values ) { ast += ` (${this.actual})` }
      return ast
    }
    else
    {
      ...
      // if ( this.isFile || this.isFolder )
      // {
      //   // log('-> isFile ou isFolder')
      //   this._actual_str = `le ${this.isFile?'fichier':'dossier'} ${this.actual}`
      // }
      // else
      // {
  ...
  **/
  realValueToStringValue (realValue)
  {
    ...
    if ( Array.isArray(realValue) )
    {
      ...
      let arr = realValue.map( (e) => { return (new Any(e)).inspect() })
      let last = arr.pop()
  ...
  **/
  ...
  {
    let the_it
    // En cas d'échec, on cherche le numérot de ligne et on mémorise
