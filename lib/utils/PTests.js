  ...
  **/
  static run ()
  {
    ...
    glob(path.join(ptests_folder,'**','*_spec.js'), (err, files) => {
      if ( err ){ throw err }
      files.map( (fpath) => {
        fpath = path.resolve(path.join('.',fpath))
        console.log("Exécution du fichier ", fpath)
        try
        {
...
// class itObjectClass
// {
//   static it (case_str, methode )
//   {
//     console.log(`it "${case_str}"`)
//     methode.call(PTests)
//     return itObjectClass
//   }
//   static context (context_str, methode )
//   {
//
//     return itObjectClass
//   }
// }
// itObjectClass.prototype.and = itObjectClass.prototype.it
//
...
class TurnObject
{
  ...
  {
    this.actual     = actual
    this.actual_str = actual_str || actual
  ...
  *** --------------------------------------------------------------------- */
  ...
  get to () {
    console.log("Entrée dans le to de", this.actual_str)
    return this
  }
  get be () {
    console.log('Entrée dans le be de', this.actual_str)
    return this
  }
  get and () {
    console.log('Entrée dans le and du case de', this.actual_str)
    return this
  ...
  {
    let m = ''
    m += this.isOK ? 'OK' : 'Erreur' // évite aussi les majuscules
    m += ', '
    m += this.actual_str
    m += this.strict ? ` {${typeof this.actual}}` : ''

    if ( this.isOK )
    {
      if ( this.positive ) { m += 'est' }
      else { m += 'n’est pas'}
    }
    else
    {
      if ( this.positive ) { m += 'devrait être'}
      else { m += 'ne devrait pas être'}
    }

    m += ' '
    m += this.verb_comparaison // par exemple 'égal' ou 'un entier'

    if ( this.expect )
    {
      m += ' '
      m += this.preposition_expect || 'à' // 'que'
      m += ' '
      m += this.expect_str
      if ( this.strict && !this.isOK ) { m += ` {${typeof this.expect}}`}
    }
    return m
  }
  /** ---------------------------------------------------------------------
    *
    *   MOTS DE FIN DE CHAINE
  ...
  *** --------------------------------------------------------------------- */
  equal (value, value_str)
  {
    ...
    // new Case(this.actual == this.value).output()
    this.expect     = value
    this.expect_str = value_str || value // TODO plus tard : "pseudo (valeur)"
    if ( this.strict ) { this.resultat_droit = this.expect === this.actual}
    else { this.resultat_droit = this.expect == this.actual}
  ...
  greater_than (value)
  {
    console.log("Entrée dans le greater_than de", this.actual)
    return this
  }
  less_than (value)
  {
