...
<span class="pend_count">${this.nombre_pendings} pending${s_pends}</span>
  ...
  *
  */
  write_case (the_it, resultat_str, okCase)
  {
    ...
    divcase.className = `case tab${the_it.tab_level} ${okCase ? 'pass' : 'fail'}`
...
class PTestContainer {
  constructor (description, owner)
  {
    ...
    // Réglage du tab_level
    let lev
    if (owner) {
      lev = parseInt(owner.tab_level)
      log(`--owner *${owner.type}* ${owner.level_type} «${owner.description}» ${owner.tab_level}`)
      log(`--this  *${this.type}* ${this.level_type} «${this.description}» ${this.tab_level}`)
      let owner_type_this_type = `${this.owner.type}-${this.type}`
      if ( 'describe-describe' == owner_type_this_type)
      {
        lev += 1
      }
      else if ( 'context-context' == owner_type_this_type )
      {
        lev += 1
      }
      else if ( 'it-context' == owner_type_this_type)
      {
        lev -= 1
      }
      if ( this.level_type == owner.level_type )
      {
        lev += 0 // ne rien ajouter
      }
      else if ( this.level_type > owner.level_type )
      {
        lev += 1
      }
      else if ( this.level_type < owner.level_type )
    ...
    this.tab_level = lev
    log(`Tab level de «${this.description}» mis à ${this.tab_level}`)

    if ( this.type != 'it' )
    {
      PTestContainer.lastOwner = this
...
class DescribeClass extends PTestContainer
{
  ...
  *** --------------------------------------------------------------------- */
  ...
  // On crée un tout nouveau describer
  static describe (description)
  {
    return new DescribeClass(description)
  }
  ...
  *** --------------------------------------------------------------------- */
  ...
  { return new DescribeClass(description, this) }
  context (description)
  { return new ContextClass(description, this) }
  it (description, methodetest)
  {
    new ItClass(description, methodetest, this).run()
    return this
  }
}
class ContextClass extends PTestContainer
{
  ...
  {
    new ItClass(description, methodetests, this).run()
    return this
  }
}
class ItClass extends PTestContainer
{
  constructor (description, methode, owner)
  {
...
DescribeClass.prototype.and = DescribeClass.prototype.it
ContextClass.prototype.and  = ContextClass.prototype.it
...
`
  ...
  *** --------------------------------------------------------------------- */
  ...
  writeCase ()
  {
    ...
    if ( ! this.options.not_a_test )
    {
      // On écrit le cas dans le document
      PTests.writer.write_case (this.it, this.returnedMessage, this.isOK)
