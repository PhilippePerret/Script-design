...
/**
* Si true, on n'utilise plus d'imbrication des describes et des contexts, tout
* le texte est mis sur une ligne.
* Exemple :
*   ONE_LINE_IT = false
*   // Produit :
*   Class Any
*     Pour les nombres
*       une égalité
*         renvoie true pour deux nombres égaux

*   ONE_LINE_IT = true
*   // Produit :
*     Class Any pour les nombres une égalité
*       renvoie true pour deux nombres égaux
**/
const ONE_LINE_IT = true
...
const FICHIER_TO_TEST = path.join('.','tests','ptests','autotests','any_spec.js')
  ...
  *** --------------------------------------------------------------------- */
  ...
  buildReturnedMessage ()
  {
    let
      ...
      , ok            = this.isOK
      , mkOK          = ok ? '<span class="ok">OK</span>' : `<span class="er">Erreur</span> line ${this.errLine}`
  ...
  **/
  addMarkSiblingConditions ()
  {
    if ( this.sibling_index ) {
      ...
      this._returned_message = `<span class="${css}">${mark}</span> ${this._returned_message}`
    }
  }
  ...
  *** --------------------------------------------------------------------- */
  ...
  equal () // cf. Note0001
  {
    ...
    { // <= Une liste
      ...
      this.resultat_droit = Array.areEqual(this.actual,this.expect)
    }
    else if ( 'object' === typeof this.actual )
    {
