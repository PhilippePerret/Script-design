...
class PTestsWaiter
{
  ...
  **/
  ...
  get isOK ()
  {
    ...
    {
      console.log("Dans le catch de isOK",erreur)
      return false
    }
  }
  ...
  waitMethod ()
  {
    ...
    else
    {
      if ( (new Date().getTime()) > this.end_time )
      {
        ...
        {
          this.error = 'Timeout dépassé'
          // console.log('Je joue run_else')
          throw new PTestsExpectationError("Le temps d'attente est malheureusement dépassé…")
          // this.run_else()
        }
        else
        {
          // On avertit que le test asynchrone est fini
          this.logMessageByCheckIf('timeout')
          throw new PTestsExpectationError("Le temps d'attente est malheureusement dépassé…")
        }
      }
    }
...
class PTestExpectObject
{
  ...
  **/
  actualTagContainsExpect ()
  {
    ...
    // console.log(`-> actualTagContainsExpect (this.actual = ${JSON.stringify(this.actual)} / this.expect = ${JSON.stringify(this.expect)})`)
    let
          domActual
        , text
        , exp   = this.expect
    ...
    try
    {
      ...
      if('string' === typeof this.actual){
        // <= this.actual est un String
        //  => Ça peut être du code HTML ou un selector. On test la première
        //     lettre pour le savoir.
        if ( this.actual.startsWith('<') )
        {
          // domActual.innerHTML = this.actual
          domActual = DOM.create('_cptests',{inner:this.actual})
        }
        else
        {
          domActual = document.querySelector(this.actual)
          if(!domActual){throw new Error(`${this.actual} n'est pas un sélecteur valide ou ne correspond à aucun élément DOM.`)}
        }
      } else {
        if ( 'function' !== typeof this.actual.getAttribute ){
          throw new PTestsExpectationError('Il faut fournir un DOMElement ou le string d’un DOMElement.')
        }
        domActual = DOM.create('_cptests')
        domActual.appendChild(this.actual)
      }

      // Nombre d'éléments à trouver.
      let count, maxcount, mincount, children
      if (exp.count    ){count    = exp.count    ; delete exp.count     }
      if (exp.max_count){maxcount = exp.max_count; delete exp.max_count }
      if (exp.min_count){mincount = exp.min_count; delete exp.min_count }
      if (exp.children ){children = exp.children ; delete exp.children  }


      // Un inner texte à trouver
      if ( exp.text || exp.inner ){
        text = exp.text || exp.inner
        delete exp.text; delete exp.inner
        // Le text doit être de type String ou RegExp
        if (['String','RegExp'].indexOf(text.constructor.name) < 0){
          throw new PTestsExpectationError(`le texte cherché doit être exclusivement de type {String} ou {RegExp} (il est de type {${text.constructor.name}})`)
        }
      }
      ...
      if ( exp.tag && exp.tag != '*' )
      {
        elementList = domActual.getElementsByTagName(exp.tag)
        capTag = `&lt; ${exp.tag.toUpperCase()} &gt;`
        // Erreur : aucun élément trouvé
        if (elementList.length == 0)
        {
          throw new PTestsExpectationError(`aucune balise ${capTag} n’a été trouvée`)
        }
        exp_str += capTag
      ...
      {
        throw new PTestsExpectationError('Le pseudo-élément « * » est interdit en valeur de tag.')
      }

      // log("this.expect (après recherche tag):",this.expect)
      // log("exp (après tag) :",exp)

      let nombre_founds, i, domel, resul, newList

      // On transforme la liste des DOMElement en vrai Array
      nombre_founds = elementList.length
      for(i=0;i<nombre_founds;++i){realElementList.push(elementList[i])}
      // log(`[have_tag] Nombre de balises ${capTag} candidates :`,realElementList.length)


      // On recherche LE TEXTE s'il est fourni dans les conditions
      // ---------------------------------------------------------
      if (text)
      {
        // log('[have_tag] Recherche du text fourni')
        for(i=nombre_founds-1;i>=0;--i){
          domel = realElementList[i]
          resul = this.strict ? Any.areEqual(domel.innerHTML,text) : Any.isContainedBy(text,domel.innerHTML)
          if ( false == resul ){ realElementList.splice(i,1) }
        }
        act_str.push(`avec le texte « ${text} »`)
        nombre_founds = realElementList.length
        // log(`[have_tag] Nombre de balises candidates restantes après recherche text : ${nombre_founds}`)
      }


      // Tout ce qui reste dans le `exp` doit être des attributs à trouver
      // dans les balises retenues
      let   attr
          , val_attr        // contiendra : "valeur attribut"
          , pair_str        // contiendra : ATTRIBUT="valeur"
          , attr_in_tag
          , act_attr_str  = []
          , attrs_in_tag  = []
          , attrs_out_tag = []
      ...
      // log('exp avant attributs',exp)

      if ( exp !== {} )
      {
        // log(`[have_tag] Recherche des attributs dans les (${nombre_founds}) candidates restantes`)

        // Recherche des ATTRIBUTS spécifiés
        // ---------------------------------
        // On tient à jour deux lists, `attrs_in_tag` et `attrs_out_tag` qui
        // font contenir respectivement les attributs trouvés et ceux non
        // trouvés, pour construire le message final explicite et précis.
        nombre_founds = realElementList.length
        let firsti = Number(nombre_founds - 1)
        for(i=firsti;i>=0;--i){

          // On prend l'élément
          domel = realElementList[i]

          // Initialisation des attributs valides et non valides, pour le
          // test et le message final. Il ne faut relever qu'une seule fois
          // les attributs
          attrs_out_tag = []
          attrs_in_tag  = []

          // On boucle sur chaque attribut
          console.log('On recherche dans l’élément suivant…', domel)
          console.log('… chaque attribut de', exp)
          for( attr in exp )
          {
            // La valeur de l'attribut `attr`
            val_attr = exp[attr]
            // Le texte qui sera mémorisé soit dans attrs_in_tag ou attrs_out_tag
            pair_str = `${attr.toUpperCase()}="${String(val_attr)}"`

            // Seulement lorsque c'est le premier tour sur les attributs, on
            // en mémorise la liste.
            if ( i == firsti )
            {
              act_attr_str.push(pair_str)
            }

            // Évaluation de la réussite en fonction de l'attribut, par exemple
            // pour la class et le style
            attr_in_tag = domel.getAttribute(attr)
            if( attr == 'class' && attr_in_tag)
            {
              let classes = attr_in_tag.split(' ')
              not_ok = classes.indexOf(val_attr) < 0
            }
            else
            {
              not_ok = !attr_in_tag || attr_in_tag != val_attr
            }

            // En fonction de la réussite, on ajoute l'attribut à la liste
            // des attributs de la balise ou non.
            // Noter que pour le moment on ne fait rien de ces listes.
            // TODO Pour l'avenir, si vraiment c'est relevant, il faudrait
            // prévoir un retour en précisant les balises qui contiennent au
            // moins un attribut mais pas tous. Le message de retour dirait
            // donc qu'aucune balise n'a été trouvée (ou quelques unes) mais que
            // quelques balises contenaient presque les attributs demandés.
            // Il faudrait dire leur valeur et la valeur attendu.
            // Noter que pour faire ça, il faudrait enregistrer des données avec
            // le DOMElement (`domel` ici) pour conserver les informations. Car
            // ici, pour le moment, on ne fait que supprimer le DOMElement de la
            // liste, basta.
            if ( not_ok ) {
              attrs_out_tag.push(pair_str)
            }
            else
            {
              attrs_in_tag.push(pair_str)
            }
          }// fin boucle sur chaque attribut

          // Une fois qu'on a bouclé sur tous les attributs sur la balise
          // courante, on peut décider s'il faut la retirer ou la conserver
          if ( attrs_out_tag.length /* des attributs invalides */)
          {
            realElementList.splice(i,1)
          }
        }//fin boucle sur chaque balise gardée
        if ( i == firsti && act_attr_str.length )
        { act_str.push(`avec ${act_attr_str.join(', ')}`)}

        nombre_founds = realElementList.length
      } // fin de s'il reste des attributs à trouver
      ...
      // log(`[have_tag] BALISES CANDIDATES FINALES : ${nombre_founds} (act:${act_str.join(' ')}, exp:${this._expect_str})`)
      if(nombre_founds){
        m = `${this.actual} contient ${act_str.join(' ')}`
        if ('' != messChildren) { m += ` et contient ${messChildren}` }
        this.options.template.success = m
      }else{
        m = `${this.actual} ne contient pas ${act_str.join(' ')}`
