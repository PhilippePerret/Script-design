  ...
  **/
  run ()
  {
    try
    {
      ...
      PTestContainer.init()
      log(`*test* require(${this.relative_path})…`)
      require(this.path) // => définit simplement les cases sans rien tester
      log('    … require(${this.relative_path}) OK')
...
<span class="pend_count">${this.nombre_pendings} pending${s_pends}</span>
  ...
  */
  write_case (the_it /* null quand sibling `and` */, iexpect)
  {
    ...
    if ( iexpect.hasPreviousExpectation ) css.push('sib')
...
class PTestExpectObject
{
    ...
    **/
    ...
    throwError ()
    {
      ...
      let oldThrowErrorFunction = throwError
      let my = this
  ...
  **/
  constructor ( actual, actual_str, options, itcase )
  {
    ...
    log(`Index de l'expectation « ${this.actual} vs ${this.expect} » dans son i : ${this.expect_index}`)
  ...
  get and () {
    ...
    // OBSOLETE avec la gestion des expect_index (TODO Supprimer si OK)
  ...
  //   iexpect._hasPreviousExpectation = true
  ...
  get hasPreviousExpectation () {
    if (undefined === this._hasPreviousExpectation)
    { this._hasPreviousExpectation = undefined !== this.previous_expect }
    return this._hasPreviousExpectation
  }
...
`
  ...
  defineActualString ()
  {
    ...
    {
      this._actual_str = this.realValueToStringValue(this.actual)
    }
  }
  ...
  realValueToStringValue (realValue)
  {
    ...
    switch(realValue.constructor.name)
    {
      ...
      case 'Function':
        ...
        let offset = actstr.indexOf('{')
        if (actstr.startsWith('class')){
          let nom = actstr.substring(5, offset - 1).trim()
          return `la class ${nom}`
        } else {
  ...
  **/
  addAmorceMessage ()
  {
    ...
    if ( this.expect_index ) {
      if ( this.isOK === this.previous_expect.isOK ) {
  ...
  **/
  writeCase ()
  {
    ...
    if ( ! this.evaluate_only )
    {
      ...
      let the_it = this.expect_index ? null /* ne pas remettre le it */ : this.it
  ...
  *** --------------------------------------------------------------------- */
  ...
  get null  () { return this.strictly.equals(null) }
