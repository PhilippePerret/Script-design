  ...
  actualTagContainsExpect ()
  {
    ...
    try
    {
      ...
      if('string' === typeof this.actual){
        ...
        if ( 'function' !== this.actual.className ){
      ...
      if ( this.expect.tag && this.expect.tag != '*' )
      {
        elementList = domActual.getElementsByTagName(this.expect.tag)
        // log('elementList');log(elementList)
        exp_str += `&lt; ${this.expect.tag.toUpperCase()} &gt;`
        act_str.push(`&lt; ${this.expect.tag.toUpperCase()} &gt;`)
        delete exp.tag
      } else if ( this.expect.tag == '*' )
      {
      ...
      // log("Valeur de exp : ",exp)

      for(attr in exp)
      {
        val_attr = exp[attr]
        // log(`Test de l'attribut ${attr} de valeur "${val_attr}"`)

        act_attr_str.push(`${attr.toUpperCase()}="${String(val_attr)}"`)
        nombre_founds = realElementList.length
        for(i=nombre_founds-1;i>=0;--i){
          domel = realElementList[i]
      ...
      if(nombre_founds){
        this.options.template.success = `${this.actual} contient ${act_str.join(' ')}`
      }else{
        throw new PTestsExpectationError(`${this.actual} ne contient pas ${act_str.join(' ')}`)
      }
