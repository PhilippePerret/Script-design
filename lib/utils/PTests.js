...
const FICHIER_TO_TEST = path.join('.','tests','ptests','autotests','equality_spec.js')
...
class PTests {
  ...
  **/
  ...
  {
    this.on_start()
    this.test_file_list.forEach( (frelpath) => {
  ...
  **/
  static get test_file_list ()
  {
    if ( undefined === this._test_file_list )
    {
      let li

      if ( FICHIER_TO_TEST ) { li = [FICHIER_TO_TEST] }
      else {
      ...
      this._test_file_list = li
    }
    return this._test_file_list
  }
...
class PTestExpectObject
{
  ...
  get and () {
    var autre_expect = new PTestExpectObject(this.actual, this.actual_str_provided, ItClass.current)
...
`
  ...
  *   condition sœur.
  **/
  addMarkSiblingConditions ()
  {
    if ( this.sibling_index ) {
      this._returned_message = `${this.isOK?'<span class="et">ET</span>':'<span class="mais">MAIS</span>'} ${this._returned_message}`
    }
  }
  ...
  *** --------------------------------------------------------------------- */
  ...
  true () {
    return this.equal(true, arguments[0], arguments[1])
  }
  ...
  get false ()
  {
    ...
    // throw new Error('La méthode `false` n’est pas implémenté')
  }
  get null ()
  {
    return this.equal(null)
    // throw new Error('La méthode `null` n’est pas implémenté')
  }
...
{
  return new PTestExpectObject(_actual, _actual_str, ItClass.current)
}
