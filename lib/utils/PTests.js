...
let
  ...
  , robot   = require('robotjs') // exposé au monde
...
class PTests {
  ...
  **/
  static on_start ( mode_in_application )
  {
    ...
    let output = new Writer(PTEST_RESULTS_SECTION_ID)
  ...
  **/
  actualTagContainsExpect ()
  {
    ...
    try
    {
      ...
      // log("domActual class = ", domActual.constructor.name)
      // log("this.expect:",this.expect)
      // log("exp avant count :",exp)
      ...
      if ( exp !== {})
      {
        ...
        if (act_attr_str.length) { act_str.push(`avec ${act_attr_str.join(', ')}`)}
        nombre_founds = realElementList.length
      ...
      if ( children )
      {
        ...
        // Si on doit plutôt utiliser une boucle for
        // for(ichild=0;ichild<nombre_children;++ichild){
      ...
      if(nombre_founds){
        this.options.template.success = `${this.actual} contient ${act_str.join(' ')} et contient ${messChildren}`
      }else{
        throw new PTestsExpectationError(`${this.actual} ne contient pas ${act_str.join(' ')} qui contiendrait ${messChildren}`)
      }
...
// Pour les simulations de clé, de souri
global.robot = robot
